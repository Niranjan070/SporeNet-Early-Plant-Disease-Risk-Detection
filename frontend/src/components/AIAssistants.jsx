...
export function FarmingAssistantPanel({ aiReady, cropType, onToast, docked = false }) {
  ...
  return (
    <article className={`assistant-card ${docked ? 'persistent-assistant-card' : ''}`}>
      <div className="card-header">
        <div>
          <p className="section-kicker">{docked ? 'Always Available' : 'Assistant One'}</p>
          <h3 className="card-title">General farming support</h3>
        </div>
        <span className={`soft-badge ${aiReady ? '' : 'muted-badge'}`}>
          {aiReady ? (docked ? 'Assistant live' : 'Gemini live') : 'Gemini unavailable'}
        </span>
      </div>

      <p className="assistant-intro">
        {docked
          ? 'Keep this assistant open while you upload images, review detections, or read the diagnosis on the left.'
          : 'Farmers can ask normal farming questions here without leaving the app.'}
      </p>
  ...
  return (
    <section className="assistants-section" id="ai-assistants">
      <div className="section-heading">
        ...
        <h2>Keep the farming assistant docked on the right while the diagnosis workspace stays on the left</h2>
        <p>
          The right rail stays available for everyday farming questions. This section focuses on the analysis-linked
          assistant that reads the current YOLO output and explains the disease signal in simple field language.
        </p>
      </div>

      <article className="assistant-card featured diagnosis-section-card">
        <div className="card-header">
          <div>
